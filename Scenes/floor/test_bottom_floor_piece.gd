...
class Test_set_transparent:
	...
	func before_each():
		...
		test_piece = prototype_script.instance()
		add_child_autofree(test_piece)


	func test_set_transparent():
		assert_null(test_piece.find_node("Cube003").material_override, "material_override is set to null on the cube itself to begin")

		test_piece.set_transparent()

		assert_eq(test_piece.find_node("Cube003").material_override, test_piece.transparent_material, "material_override is set to transparent after set_transparent is called")
	...
	func test_set_opaque():
		...
		assert_null(test_piece.find_node("Cube003").material_override, "material_override is set to null after set_opaque is called")
	...
	func test_add_wall_at_edge():
		...
		assert_true(test_piece.find_node("wall2").is_visible(), "wall set to visible after add_wall_at_edge is called")
	...
	func test_hide_walls():
		...
		assert_false(test_piece.find_node("wall2").is_visible(), "wall set to invisible after hide_walls is called")
